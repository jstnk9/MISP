...
def main(args):
    ...
    galaxyCluster = create_cluster(uuidGalaxy)
        ...
        for dirpath, dirs, files in os.walk(pathsigma):
            ...
            for f in files:
                if f.endswith(".yml"):
                    ...
                    valuesData.append(parse_sigma_to_cluster(jsonData, f, path.split("rules")[1]))
...
def parse_sigma_to_cluster(jsonData, sigmaFile, sigmaPath):
    ...
    if jsonData.get("references"):
        ...
        valueData["meta"]["refs"] = [*set(valueData["meta"]["refs"])] # Removing duplicated references
    ...
    valueData["meta"]["falsepositive"] = jsonData.get("falsepositives", "No established falsepositives")
    valueData["meta"]["refs"].append("https://github.com/SigmaHQ/sigma/tree/master/rules%s/%s"%(sigmaPath.replace("\\", "/"), sigmaFile)) #this value only works if you set the path like it was cloned from github
    valueData["meta"]["logsource.category"] = jsonData.get("logsource").get("category", "No established category")
    valueData["meta"]["logsource.product"] = jsonData.get("logsource").get("product", "No established product")
    return valueData
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description="This script can convert your sigma rules in MISP galaxies, generating both files needed for cluster and galaxies. If you need more information about how to import it, please, go to https://github.com/jstnk9/MISP/tree/main/misp-galaxy/sigma")
	parser.add_argument("-p", "--path", dest="inputPath", required=True, default="None", help="Path with your sigma rules.")
	parser.add_argument("-r", "--recursive", dest="recursive", action="store_true", help="If you have subfolders on the initial path and you want to convert all of them, use -r to do it recursive.")
